        ...
        await Player.findByIdAndUpdate(this.playerID, {
          ...
          $inc: {
            [`points.${this.hertz <= 60 ? "lrr" : "hrr"}`]:
              Math.round(100 * (justOne === 1 ? -(this.percent < 100 && (level?.position! >= 50 && level?.listpercent! <= this.percent) ? level?.points! / 6 : level?.points!)! : 0)) / 100,
            ["points.comb"]: Math.round(100 * (justOne === 1 ? -(this.percent < 100 && (level?.position! >= 50 && level?.listpercent! <= this.percent) ? level?.points! / 6 : level?.points!)! : 0)) / 100,
          },
        }).session(session);
...
recordSchema.pre("save", async function () {
  ...
  const player = await Player.findOneAndUpdate(
    { name: this.player },
    {
      $addToSet: { records: this._id },
      $inc: {
        [`points.${parseInt(this.hertz.split("/").at(-1) as string) <= 60 || this.hertz.split("/").at(-1) as string == "CBF" ? "lrr" : "hrr"}`]: (this.percent < 100 ? level.points! / 6 : level.points!),
        ["points.comb"]: (this.percent < 100 ? (level.points! / 6) : level.points!),
      },
    },
    { new: true }
  ).session(session as ClientSession);
