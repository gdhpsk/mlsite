        ...
        await Player.findByIdAndUpdate(this.playerID, {
          ...
          $inc: {
            [`points.${this.hertz <= 60 ? "lrr" : "hrr"}`]:
              Math.round(100 * (justOne === 1 ? -(this.percent < 100 ? level?.points! / 6 : level?.points!)! : 0)) / 100,
            ["points.comb"]: Math.round(100 * (justOne === 1 ? -(this.percent < 100 ? level?.points! / 6 : level?.points!)! : 0)) / 100,
          },
        }).session(session);
