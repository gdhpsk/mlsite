...
let faq: Record<"records" | "submissions" | "misc", Array<[string, React.ReactNode]>> = {
  ...
  ["Will you ever extend the list to 150 levels?", <>Unfortunately due to calculation and workload issues, we <b>do not have plans to extend the list in the near future</b>. However, P8geons runs an unofficial extension of the list at <b><a href="https://discord.gg/6J293BBvb7" style={{textDecoration: "none"}} target="_blank">Mobile List Plus</a></b>, so do check it out!</>],
...
const FAQ: React.FC = () => {
  let [viewable, setViewable] = useState(false)
  useEffect(() => {
    (async () => {
      let req = await fetch("https://comeyourmomjiggler.hpsk.me/users")
      let json = await req.json()
      faq.misc[0] = ["Does the Mobile List have a discord server?", <>There is, {json.count} members and counting, but you’ll have to find it!</>]
      setViewable(true)
    })()
  }, [])
  useEffect(() => {
  ...
  return (
    <div className="rounded-box max-w-5xl border-4 bg-[#f2f7ff] lg:p-8 sm:m-12 sm:mx-auto">
        <div className={`rounded-box flex w-4/5 flex-col overflow-y-auto bg-white p-4 py-12 shadow-inner`}  style={{width: "-webkit-fill-available"}}>
            ...
            {viewable && faq.submissions.map(e => <>
              <br></br>
              <h1 className="font-extrabold text-xl list-item ml-5">{e[0]}</h1>
              <h1 className="text-lg">{e[1]}</h1>
            </>)}
